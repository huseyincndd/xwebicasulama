...
export default async function SubcategoryPage({ params }: PageProps) {
  ...
  // Alt kategoriyi bul
  const currentSubcategory = mainCategory.subcategories.find(sub => sub.slug === subcategory);
  if (!currentSubcategory) {
    return notFound();
  }

  // Bu alt kategorideki ürünleri filtrele (şimdilik tüm ürünleri göster)
  const subcategoryProducts = mockProducts.slice(0, 12); // Şimdilik ilk 12 ürünü göster

  // Alt kategori iconlarını belirleme fonksiyonu
  const getSubcategoryIcon = (subcategorySlug: string) => {
    if (subcategorySlug.includes('sprinkler')) return <Droplets className="h-8 w-8 text-white" />;
    if (subcategorySlug.includes('abot')) return <Wrench className="h-8 w-8 text-white" />;
    if (subcategorySlug.includes('boru')) return <Settings className="h-8 w-8 text-white" />;
    if (subcategorySlug.includes('kaval')) return <Zap className="h-8 w-8 text-white" />;
    if (subcategorySlug.includes('damla')) return <Droplets className="h-8 w-8 text-white" />;
    if (subcategorySlug.includes('ek-parcalar')) return <Settings className="h-8 w-8 text-white" />;
    if (subcategorySlug.includes('damlatilar')) return <Droplets className="h-8 w-8 text-white" />;
    if (subcategorySlug.includes('filtreler')) return <Filter className="h-8 w-8 text-white" />;
    if (subcategorySlug.includes('ev-tipi')) return <Home className="h-8 w-8 text-white" />;
    if (subcategorySlug.includes('hortum')) return <Waves className="h-8 w-8 text-white" />;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-8">
          ...
          <Link href={`/categories/${slug}`} className="hover:text-blue-600 transition-colors">{mainCategory.name}</Link>
          <ChevronRight className="h-4 w-4" />
          <span className="text-gray-900 font-medium">{currentSubcategory.name}</span>
        </nav>

        {/* Subcategory Header - Arkaplan Resmi ile */}
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8 border border-gray-100 relative">
              ...
              <div className="mb-6 lg:mb-0">
                <div className="flex items-center space-x-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-blue-100 text-sm">{mainCategory.name} &gt; {currentSubcategory.name}</p>
                  </div>
                </div>
                
                <p className="text-white/90 text-lg leading-relaxed max-w-2xl mb-4">
                  Yüksek kaliteli {currentSubcategory.name.toLowerCase()} ürünleri en uygun fiyatlarla. 
          ...
          <div className="absolute bottom-4 left-4 w-20 h-20 bg-blue-400/20 rounded-full blur-lg"></div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-8 border border-gray-100">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-gray-600 text-sm">{subcategoryProducts.length} ürün gösteriliyor</span>
